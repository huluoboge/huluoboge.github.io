...
def plot_probability_comparison():
    ...
    plt.plot(n_values, approx_probs, 'r-', label='近似概率 Pₐ = pᵏ', linewidth=2)
    plt.plot(n_values, exact_probs, 'b-', label='精确概率 Pₑ', linewidth=2)
    plt.xlabel('总测量数 (n)')
    plt.ylabel('全内点采样概率')
    plt.title(f'近似vs精确概率比较 (p={p}, k={k})')
    plt.legend()
...
def plot_iteration_comparison():
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(p_values, approx_iters, 'r-', label='近似方法', linewidth=2)
    plt.plot(p_values, exact_iters, 'b-', label='精确方法', linewidth=2)
    plt.xlabel('内点比例 (p)')
    plt.ylabel('所需迭代次数')
    plt.title(f'迭代次数比较 (n={n}, k={k}, s={s})')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.yscale('log')
    plt.tight_layout()
