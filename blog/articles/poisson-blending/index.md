...
[Poisson 表面重建](../poisson_understand_2/) 使用同一类数学骨架。两边都是由一个向量场恢复与它最相容的标量场。图像融合里，标量场是亮度，向量场是引导梯度；表面重建里，标量场是隐式函数，向量场是法向场。差别主要在定义域、边界条件和离散方式，方程形态保持一致。
