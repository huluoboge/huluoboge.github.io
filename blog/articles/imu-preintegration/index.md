...
理解预积分因子，需要明确，残差是**同一个相对运动由两种信息来源计算出来之后，它们之间的不一致**。
...
**使用已经预积分的 IMU 测量。** 原始 IMU 在参考 bias 下形成
