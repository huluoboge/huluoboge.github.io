...
# ESKF（误差状态卡尔曼滤波）入门：从状态预测到误差注入

> 这是 ESKF 系列的第一篇。ESKF 是 Error-State Kalman Filter 的缩写，中文通常译为“误差状态卡尔曼滤波”。本文不要求读者先了解 SfM、SLAM、因子图或 IMU 预积分，而是从一个基本问题开始：状态不能直接读出时，滤波器如何用模型和观测逐步把它估计出来？

**阅读路线：** 本文先建立完整的 ESKF 闭环；接着阅读 [ESKF 误差动力学：从 IMU 模型推导 $F$ 和 $G$](../eskf-error-dynamics/index.html)，把传播阶段的矩阵从 IMU 方程中推出来；再阅读 [ESKF 观测更新：从观测模型推导 $H$ 矩阵](../eskf-observation-h/index.html)，学习不同传感器如何进入更新阶段；最后阅读 [ESIKF：误差状态上的迭代卡尔曼更新](../esikf-iterated-error-state/index.html)，理解强非线性观测下的一次观测内部迭代。

## 摘要

机器人无法直接读出自己的真实运动状态。通常先用运动模型预测，再用带噪声的传感器观测修正。位置和速度可以用普通向量表示，姿态则属于旋转空间，不能把一个三维向量直接加到旋转矩阵上。

ESKF（Error-State Kalman Filter，误差状态卡尔曼滤波）维护两层信息：一层是按完整非线性模型传播的**名义状态**，另一层是描述名义状态附近不确定性的**误差状态**。滤波器在线性误差空间中完成 Kalman 更新，再把得到的小修正注入名义状态，并重新定义误差坐标。这样，非线性运动、局部线性估计和旋转状态的几何约束可以同时保留下来。

全文只追踪一条信息流：先用运动模型传播名义状态和协方差，再用外部观测构造残差并更新误差，最后把误差注入名义状态并完成 reset。IMU 的具体 $F/G$ 推导和观测 $H$ 矩阵会在后两篇中展开；本文先把它们在算法中的位置讲清楚。
...
本文先保留 ESKF 最内核的结构，具体模型沿着同一条闭环继续展开：
...
7. **ESIKF 与优化方法**：误差状态更新如何在一次观测内部迭代，以及它与预积分、因子图和滑动窗口优化的联系与区别。

前两项内容已经分别整理成系列文章，阅读顺序与算法执行顺序一致：先理解本篇的状态组织，再推导 IMU 传播，最后推导观测更新。这样，后面看到 $F$、$G$、$H$ 和 reset Jacobian 时，它们都会对应到闭环中的一个明确步骤。

- [下一篇：ESKF 误差动力学：从 IMU 模型推导 $F$ 和 $G$](../eskf-error-dynamics/index.html)
- [再下一篇：ESKF 观测更新：从观测模型推导 $H$ 矩阵](../eskf-observation-h/index.html)
- [第四篇：ESIKF：误差状态上的迭代卡尔曼更新](../esikf-iterated-error-state/index.html)
...
这条主线也适用于 IMU、VIO 和惯性导航。后续推导不需要先背完整矩阵：只要先写清楚模型、误差定义和坐标约定，再从动力学得到 $F/G$，从观测模型得到 $H$，就能把公式放回 ESKF 的运行闭环中。对于 LiDAR 和视觉这类强非线性观测，还可以继续阅读 ESIKF，把一次观测更新展开成多轮局部误差状态迭代。
