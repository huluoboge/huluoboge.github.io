...
视觉、LiDAR 等传感器读到的是运动与环境相互作用后留下的结果。它们通常不能直接告诉我们机器人位于什么位置，而是通过像素、距离或几何关系给状态施加约束。这类传感器输出通常作为观测（measurement/observation）进入滤波器。
...
因此，“预测”和“观测”说的是信息进入估计器时扮演的角色，不是“可以预测”和“不能预测”的区别。在本文的惯性状态估计语境中，IMU 和轮速计通常用于状态传播，视觉和 LiDAR 通常用于观测更新。但这不是传感器类别的硬性规定：轮速计也可以作为速度观测，视觉里程计也可以提供相对运动约束。关键在于它在模型中被放在什么位置。

所以问题不是“从某个传感器读出真实状态”，而是：
...
> **滤波器不会直接接受观测，而是根据当前不确定性，用残差计算出相应大小的状态修正。**
...
对于位置、速度和传感器零偏，这种表示通常没有问题。但姿态不是普通的三维向量。旋转矩阵必须满足正交性和行列式约束；四元数还必须保持单位范数。直接进行
...
ESKF 关注的不是把真实状态改写成普通向量，而是：

> **在适合每种状态的局部坐标中描述真实状态相对于名义状态的偏差。**

### 3.4 一个直观类比

例如，我们估计某个物体位于 $100$ 米，真实位置可能是 $100.2$ 米。此时只估计 $0.2$ 米的局部误差，再修正原估计，就比重新描述整个位置更自然。

姿态也遵循这个思路，但修正方式不是“矩阵加上矩阵”，而是沿着一个小旋转转动当前姿态。
...
过程噪声在名义状态传播中取均值 $0$。它的统计影响通过协方差传播体现，而不是作为已知量直接加到名义状态上。

以惯性状态为例，陀螺仪读数推动姿态变化，加速度计读数经过姿态变换后推动速度和位置变化，零偏按自身模型传播。这里使用的是非线性运动方程，不是把所有状态压成一个线性向量后统一相加。
...
其中 $Q_{d,k}=\operatorname{Cov}(w_{d,k})$ 是离散过程噪声的协方差。它不是连续时间噪声强度 $Q_c$ 的简单替代，而是经过采样间隔和离散化后得到的量。

即使名义状态沿确定性的预测轨迹前进，不确定性仍会受到运动模型和传感器噪声的影响而扩大。

$F$ 和 $G$ 来自模型推导，而不是凭空指定的“滤波器矩阵”：
...
其中 $\operatorname{Log}$ 是 $\operatorname{Exp}$ 在局部邻域内的逆映射。这说明旋转误差的 reset 不是普通向量减法。只有在小角度一阶近似下，才可以把它看成“误差减去更新量”。
...
ESKF 与 EKF 并不是两套互不相关的概率推理。更准确地说，ESKF 通常把 EKF 的局部线性估计应用在误差状态上。
...
所以，ESKF 不能简单理解成“不是 EKF”。更准确的说法是：

> **ESKF 是一种误差状态形式的 EKF；它的关键区别在于状态误差如何定义、如何注入，以及如何在非欧氏状态上保持局部表示的一致性。**
...
名义姿态通过旋转复合更新，而不是使用矩阵加法。旋转的结构约束因此由状态表示本身保持。
...
三维旋转只有三个局部自由度。使用三维旋转误差，而不是直接估计旋转矩阵的九个元素，可以避免冗余参数和约束处理问题。

## 8. ESKF 不是一组可以脱离约定的万能公式
...
误差状态卡尔曼滤波的重点不是多记几条 Kalman gain 公式，而是看清它如何组织状态和误差：

- 名义状态按照完整的非线性模型传播；
- 误差状态描述真实状态相对名义状态的小偏差；
- 协方差描述这个小偏差的不确定性；
- 观测更新估计误差状态，而不是直接替换名义状态；
- 误差通过适合状态空间的方式注入名义状态；
- 注入后重新定义误差，并同步处理协方差。
