...
**阅读路线：** 本文先建立完整的 ESKF 闭环；接着阅读 [ESKF 误差动力学：从 IMU 模型推导 $F$ 和 $G$](../eskf-error-dynamics/index.html)，把传播阶段的矩阵从 IMU 方程中推出来；最后阅读 [ESKF 观测更新：从观测模型推导 $H$ 矩阵](../eskf-observation-h/index.html)，学习不同传感器如何进入更新阶段。
...
这条主线也适用于 IMU、VIO 和惯性导航。后续推导不需要先背完整矩阵：只要先写清楚模型、误差定义和坐标约定，再从动力学得到 $F/G$，从观测模型得到 $H$，就能把公式放回 ESKF 的运行闭环中。
