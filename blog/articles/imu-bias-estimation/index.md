...
# IMU 零偏可观性：静止、运动与姿态激励

> IMU 零偏能否正确估计出来，其实并不是优化器收敛了就认为已经解决了。 实际上收敛的结果不一定完全正确。 具体是否退化、稳定，需要看数据中是否有足够的独立信息。静止时，真实角速度为零，陀螺仪零偏可以直接得到；加速度计却会把重力投影和零偏混在一起。进入运动后，外部旋转参考和姿态激励会改变这种信息结构，决定零偏最终能否被分离。

本文先从测量模型出发，说明零偏进入系统的方式；再分别讨论陀螺仪和加速度计的退化条件；随后用 Jacobian 的秩和条件数区分“不可观”和“虽然可观但很不稳定”；最后给出联合求解方法，并用手持三维扫描说明这些结论怎样影响采集动作。如果还不熟悉 IMU 噪声参数，可先看 [IMU 噪声参数：噪声密度、角度随机游走、速度随机游走与零偏不稳定性](../imu-noise-allan-variance/index.html)；如果想了解零偏如何进入关键帧约束，再看 [IMU 预积分详解：原理、推导与 C++/Eigen 实现](../imu-preintegration/index.html)。

本文最后的结论：静止段可以初始化陀螺仪零偏；单一姿态下不能仅凭加速度计均值分离水平零偏和倾斜；运动中的陀螺仪需要独立的旋转参考；加速度计需要姿态变化来改变重力投影。是否可观由 Jacobian 的零空间决定，估计是否可靠则由最小奇异值和条件数决定。
...
本文从统一的 IMU 测量模型出发，分别讨论角速度零偏和加速度计零偏的可观性。对于陀螺仪，静止条件提供真实角速度为零的独立参考，运动条件则需要外部旋转约束；对于加速度计，重力投影与零偏在单一姿态下耦合，必须通过姿态变化获得额外信息。理论部分最后会落到联合求解和采集设计，手持扫描只是帮助理解这些条件的一个具体例子。
...
在 SLAM、VIO 和 LIO 中，IMU 常被用来连接相邻时刻的姿态、速度和位置。传感器频率高、短时间内积分稳定，但零偏会把一个很小的恒定误差变成长期漂移。这里先不限定具体设备，只关心一个一般状态估计问题：给定原始 IMU、可用的外部相对约束和重力信息，判断零偏与姿态是否能够被区分。

状态、输入和约束的角色可以先分开：

- 位姿、速度和零偏是待估计状态；
- 原始陀螺仪和加速度计读数是高频输入；
- 视觉、LiDAR 配准、回环或其他外部传感器提供相对几何约束；
- 重力提供世界系中的绝对竖直方向，但不能提供 yaw 或全局平移；
- 静止片段和运动过程中的姿态变化决定零偏是否具有足够的独立观测。

IMU 传感器频率高、短时间内积分稳定，但零偏会把一个很小的恒定误差变成长期漂移。两种零偏的表现不同：
...
下面的分析都采用同一坐标约定。先写清楚测量模型，才能判断哪些量可以由数据区分；再分别分析陀螺仪和加速度计，最后回到联合优化和具体采集案例。陀螺仪零偏会改变姿态，而姿态又决定重力在机体系中的投影，因此不能把两个零偏当成互不相干的校准参数。

## 2. 统一测量模型与旋转记号

上一节确定了问题中的信息来源和未知量。要判断可观性，下一步不是马上运行优化，而是把坐标系、测量方程和动力学写成统一形式；后面所有秩条件和退化结论都从这里出发。
...
测量模型已经把陀螺仪零偏放在 $	ilde{\boldsymbol\omega}-\mathbf b_g$ 中。先分析它，因为静止片段可以给出一个直接的角速度参考；得到这个基准后，再看运动状态下为什么仍需要外部旋转约束。
...
陀螺仪部分的结论是：静止可以初始化零偏，运动中则需要独立的旋转信息。加速度计更复杂，因为静止时它同时测到重力投影和零偏；下面先分析这个单姿态退化，再看姿态变化如何提供额外约束。
...
这一步把前两章连接起来：陀螺仪零偏的可观性决定姿态传播是否可靠，加速度计零偏的可观性又依赖重力投影是否正确。因此，单独估计其中一个参数并永久固定，可能会把一个状态的误差转移到另一个状态。
...
前面的推导给出了可观性条件，但条件本身还不是求解器。要在实际数据上使用这些结论，需要把关键帧状态、IMU 预积分、外部相对位姿、重力和零偏随机游走写进同一个联合目标。
...
## 7. 应用例子：手持三维扫描仪中的采集设计

前一节讨论了一个与具体设备无关的联合估计模型。现在把它放到手持三维扫描仪上：扫描仪在人的手中移动和转动，同时由相机或 LiDAR 采集环境；视觉或 LiDAR 前端提供相对轨迹，原始 IMU 提供高频运动信息。这个案例的任务不是重新定义理论，而是把前面得到的秩条件和条件数翻译成采集动作。

手持扫描仪的采集动作本身就是估计问题的一部分：需要有意识地改变姿态，让不同时间段的重力投影产生足够差异，同时保留连续的场景重叠和可靠的几何约束。优化器只能利用已经采集到的信息，不能从先验或数值技巧中凭空创造可观性。

### 7.1 通过手持动作增加独立约束

下面的动作不是为了让轨迹“看起来丰富”，而是分别对应前文的可观性条件：

1. 开始时保持设备稳定数秒，估计陀螺仪零偏和噪声；
2. 在保持场景重叠的前提下，改变手持扫描仪的朝向，进行多航向旋转，最好包含转向—返回动作；
3. 在序列中加入若干不同朝向的短暂停留，使重力投影在机体系中发生可控变化；
4. 正常扫描时保持一定转弯和回转密度，避免整段轨迹只有同一朝向的长距离直行；
5. 结束时再次静止，用于检查陀螺仪零偏是否发生明显漂移，并比较首尾静止段的一致性。

对加速度计零偏而言，转动不一定要伴随剧烈线加速度。只要手持扫描仪的姿态改变，固定世界倾斜在机体系中的投影就会改变。尤其是多航向旋转和转向—返回动作，能够让堆叠 Jacobian 的列方向产生更大差异，降低加速度计零偏与重力倾斜分离问题的条件数。适度的 pitch/roll 变化还可以补充航向旋转没有覆盖的方向，但应以扫描重叠和前端跟踪稳定为前提。

### 7.2 采集动作与可观性条件的对应关系

| 手持动作或数据条件 | 主要增加的信息 | 不能单独解决的问题 |
|---|---|---|
| 开始和结束静止 | 真实角速度为零，初始化和检查陀螺仪零偏 | 单一朝向下无法分离水平加速度计零偏与倾斜 |
| 不同朝向的静止停留 | 改变重力在机体系中的投影，帮助分离加速度计零偏 | 仍需要可靠的姿态/相对轨迹关联 |
| 多航向旋转 | 提供外部相对旋转，约束陀螺仪零偏；改变水平重力投影 | 若旋转范围很小，虽可能满秩但条件数仍然很差 |
| 转向—返回 | 让平均旋转更接近零，改善水平分离的条件数 | 不能代替场景几何约束和时间同步 |
| 长距离同朝向直行 | 增加轨迹长度和场景覆盖 | 对零偏—重力分离提供的独立信息很弱 |

这张表把“建议怎样采集”直接对应到“增加了哪一列 Jacobian 信息”。如果某个动作无法改善对应的秩或条件数，就不能把它当作解决零偏可观性问题的有效手段。
...
对手持扫描仪而言，离线后处理的价值在于可以利用整段扫描中的多次转向、回转和不同朝向停留，统一评估 Jacobian 的秩、最小奇异值和条件数，再决定零偏估计是否可信；它不能替代本来就没有发生过的姿态激励。

## 8. 结论

回到手持扫描仪这个案例，角速度零偏和加速度零偏都叫 bias，但它们的可观性完全不同。采集动作也不是附加的工程经验，而是决定这些状态能否被分离的一部分观测设计；前文的理论给出了判断这种设计是否有效的依据。
