...
> IMU 零偏能不能估计出来，不是优化器收敛了就算解决，而要看数据中是否有足够的独立信息。静止时，真实角速度为零，陀螺仪零偏可以直接得到；加速度计却会把重力投影和零偏混在一起。进入运动后，外部旋转参考和姿态激励会改变这种信息结构，决定零偏最终能否被分离。
...
先记住全文的判断标准：静止段可以初始化陀螺仪零偏；单一姿态下不能仅凭加速度计均值分离水平零偏和倾斜；运动中的陀螺仪需要独立的旋转参考；加速度计需要姿态变化来改变重力投影。是否可观由 Jacobian 的零空间决定，估计是否可靠则由最小奇异值和条件数决定。
