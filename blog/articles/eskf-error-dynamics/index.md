...
### 5.3 速度误差：姿态和 accelerometer bias 的作用
...
这张表把两篇推导接到同一条闭环上：$F$、$G$ 负责把 IMU 引入的不确定性传播到观测时刻；下一篇的 $H$ 负责说明当前观测对哪些误差敏感；Kalman gain 决定两类信息如何加权；注入和 reset 则把局部计算结果接回下一轮传播。

## 9. VINS 和 FAST-LIO2 的组织方式

本文的主线是基本 ESKF 的 IMU 传播。VINS 和 FAST-LIO2 只作为两个参照，帮助区分局部动力学推导与完整系统的组织方式。
