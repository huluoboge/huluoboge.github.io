...
它回答的是：在两次外部观测之间，误差会怎样变化，以及不确定性如何被带到当前时刻。

当相机、GNSS、轮速计或 LiDAR 的新数据到来时，滤波器需要把这条测量接到当前误差状态上：
...
前面的公式给出了更新形式。它们来自一个线性高斯模型，而 ESKF 的工作是先把具体的非线性观测在当前误差状态附近变成这个模型。
